...
OpenAI clients   → :4001 (aproxy, auth + audit) → :11434 (Ollama /v1/chat/completions)
...
OpenAI-compatible:
- `POST /v1/chat/completions` — Chat Completions API, обычный и SSE-streaming режимы
...
- проксирование `POST /v1/chat/completions` в обычном и streaming-режиме;
...
{"ts":"2026-05-19T08:54:31.123456+00:00","key":"sergey","method":"POST","path":"/v1/chat/completions","model":"glm-5.2:cloud","api_family":"openai","status":200,"tokens":{"input_tokens":90,"output_tokens":24,"total_tokens":114}}
...
- `api_family` — `openai` для OpenAI-compatible routes, `ollama` для native Ollama routes, отсутствует для старых Anthropic-compatible records
...
Для `/v1/chat/completions` audit-запись также создаётся после ответа upstream или
завершения SSE-стрима. OpenAI usage нормализуется так:
`prompt_tokens` → `input_tokens`, `completion_tokens` → `output_tokens`;
`total_tokens` сохраняется в audit-записи, но не используется в counters.
...
`/api/chat`, `/api/generate`, `/api/embed`, `/api/tags`, `/api/show`,
`/api/ps`, `/api/version`),
