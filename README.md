...
| `MODELS_FILE` | `/home/sergey/Projects/aproxy/models.json` | Путь к файлу с маппингом Anthropic model IDs на Ollama |
| `APROXY_KEY_RELOAD_INTERVAL` | `1.0` | Интервал в секундах между проверками изменений `keys.json` и `models.json` |
...
живёт в `models.json` (путь задаётся `MODELS_FILE`) и перезагружается без
перезапуска сервиса.
